...
async function markHumanReview(
  ...
  const tags = Array.from(new Set([
    ...currentTags.filter((tag) => !["devir-ready","listo-publicar"].includes(tag)),
    "devir",
    "devir-review",
  ...
  await upsertProductFields(config, productId, defs, {
    "devir.review_status": "⚠ REQUIERE INTERVENCIÓN HUMANA",
    "devir.review_reasons": reasons.join(" · "),
    "devir.last_sync_at": new Date().toISOString(),
  });
}
...
async function preparePublishBatch(
  ...
  const { data, error } = await supabase
    .from("devir_sync_catalog")
    .select("supplier_sku,name,source_url,spree_product_id,spree_variant_id")
    .not("spree_product_id", "is", null)
    .order("supplier_sku")
    .range(offset, offset + limit - 1);
  if (error) throw error;
  ...
  const processRow = async (row: Record<string, unknown>) => {
    ...
    if (!sku || !productId || !variantId || !sourceUrl) return;

    try {
      const [spreeProduct, html] = await Promise.all([
        spreeRequest<SpreeProduct>(config, "GET", "/products/" + encodeURIComponent(productId)),
        devirFetch(config, sourceUrl),
      ]);
      if (spreeProduct.status === "archived") return;

      const live = parseProduct(html, sourceUrl);
      const baseReasons: string[] = [];
      if (!live) baseReasons.push("La ficha ya no devuelve un producto válido en Devir");
      else if (live.sku.trim() !== sku.trim()) baseReasons.push("El SKU de Devir ya no coincide");

      if (!live || baseReasons.length) {
        await markHumanReview(config, productId, defs, spreeProduct.tags ?? [], baseReasons);
        review += 1;
        return;
      }
      ...
      const category = categories.find((item) => item.permalink === key);
      if (!category) baseReasons.push("Categoría no encontrada");
      if (!live.purchasePrice || live.purchasePrice <= 0) baseReasons.push("Coste Devir inválido");
      if (isPack(live)) baseReasons.push("Pack que requiere comprobar unidades/contenido");
      if (isFixedPriceCandidate(live, key)) {
        baseReasons.push("Posible libro sujeto a precio fijo: validar PVP editorial");
      }

      if (live.availability !== "available" && live.availability !== "preorder") {
        baseReasons.push("Devir no lo marca disponible/preventa");
      }

      if (baseReasons.length) {
        await markHumanReview(config, productId, defs, spreeProduct.tags ?? [], baseReasons);
        if (live.availability === "unavailable" || live.availability === "unknown") unavailable += 1;
        review += 1;
        await supabase
          .from("devir_sync_catalog")
          .update({
            supplier_status: live.availability,
            snapshot: live,
            last_error: baseReasons.join(" · "),
            updated_at: new Date().toISOString(),
          })
          .eq("supplier_sku", sku);
        return;
      ...
      const margin = DEFAULT_CATEGORY_MARGINS[key] ?? 0.12;
      const pricing = competitivePrice(live.purchasePrice!, margin);
      const shipping = shippingEstimate(live, key);

      // Repair stale grouped variant ids by SKU before writing.
      try {
        await spreeRequest(
          config,
          "GET",
        ...
        const repaired = variants.find((variant) => variant.sku?.trim() === sku.trim());
        if (!repaired) throw new Error("No se encuentra la variante por SKU");
        variantId = repaired.id;
      }

      const tags = Array.from(new Set([
        ...(spreeProduct.tags ?? []).filter((tag) =>
          !["devir-review","REQUIERE-INTERVENCION-HUMANA"].includes(tag)
        ),
        "devir",
        "devir-ready",
        "publicado-validado-devir",
      ...
      await spreeRequest(
        ...
        {
          sku,
          cost_price: live.purchasePrice,
          cost_currency: "EUR",
          weight: shipping.weight,
          width: shipping.width,
          height: shipping.height,
          depth: shipping.depth,
          weight_unit: "kg",
          dimensions_unit: "cm",
          track_inventory: true,
          preorderable: live.availability === "preorder",
          preorder_ships_at: live.availability === "preorder" ? live.releaseDate : null,
          backorder_limit: null,
          prices: [{ currency: "EUR", amount: pricing.retail }],
        },
      );

      await syncBackorderability(config, variantId, "available");

      await upsertProductFields(config, productId, defs, {
        "devir.supplier_sku": sku,
        "devir.source_url": sourceUrl,
        "devir.category_key": key,
        "devir.availability": live.availability,
        "devir.release_date": live.releaseDate ?? undefined,
        "devir.review_status": "✅ VALIDADO Y PUBLICADO",
        "devir.review_reasons": "none",
        "devir.last_sync_at": new Date().toISOString(),
        "pricing.applied_margin": margin,
        "pricing.effective_margin": pricing.effective,
        "pricing.rule_source": "competitive_floor_after_vat",
        "pricing.vat_rate": 0.21,
        "pricing.cost_includes_vat": false,
        "pricing.last_synced_price": pricing.retail,
  ...
  // Keep both Devir B2B and Spree comfortably below burst limits.
