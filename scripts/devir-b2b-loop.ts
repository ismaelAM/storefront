...
async function syncOnce(): Promise<boolean> {
  ...
  const scanCode = await run("pnpm", ["devir:scan"]);
  if (scanCode !== 0) {
    console.error(
...
async function main(): Promise<void> {
  ...
  console.log(
    `Sincronizador Devir en modo continuo: ejecución inmediata y después cada ${intervalHours} h mientras este proceso y el Codespace sigan activos.`,
  );
