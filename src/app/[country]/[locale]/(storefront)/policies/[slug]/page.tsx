...
type PolicyVisualConfig = {
  ...
  contentWidth: "medium" | "large" | "full"; alignment: "left" | "center"; spacing: "compact" | "normal" | "large";
};
...
export async function generateMetadata({ params }: PolicyPageProps): Promise<Metadata> {
  ...
  return {
    title: policy.name,
    description,
    ...(localizedAlternates ? { alternates: { canonical: localizedAlternates.canonical, languages: localizedAlternates.languages } } : {}),
    openGraph: { title: policy.name, description, ...(localizedAlternates ? { url: localizedAlternates.canonical } : {}) },
  };
}
...
export default async function PolicyPage({ params }: PolicyPageProps): Promise<React.JSX.Element> {
  ...
  const rawProps = data.content.find((item) => item.type === "PolicyChrome")?.props as Partial<PolicyVisualConfig> | undefined;
  ...
  const widthClass = config.contentWidth === "medium" ? "max-w-3xl" : config.contentWidth === "full" ? "max-w-none" : "max-w-7xl";
  const spacingClass = config.spacing === "compact" ? "py-6" : config.spacing === "normal" ? "py-10" : "py-16";

  return (
    <div className={`mx-auto w-full px-4 sm:px-6 lg:px-8 ${spacingClass}`} style={{ backgroundColor: config.backgroundColor, color: config.textColor }}>
      <div className={`${widthClass} mx-auto`}>
        <header style={{ textAlign: config.alignment }}>
          <h1 className="text-3xl font-bold" style={{ color: config.titleColor }}>{config.title}</h1>
          {config.intro && <p className="mt-3">{config.intro}</p>}
        </header>
        {policy.body_html ? (
          <div className="prose prose-gray mt-8 max-w-none" dangerouslySetInnerHTML={{ __html: policy.body_html }} />
        ) : policy.body ? (
          <div className="prose prose-gray mt-8 max-w-none whitespace-pre-wrap">{policy.body}</div>
        ) : (
...
function policyTranslationFingerprint(policy: { name: string; slug: string; body: string | null; body_html: string | null }): string {
  return translationFingerprint(policy.name, policy.slug, policy.body, policy.body_html);
}
