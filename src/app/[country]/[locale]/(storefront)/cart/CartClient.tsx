      ...
      >
        ...
        >
          ...
          <h1 className="mt-4 text-2xl font-bold">
            {config.emptyTitle || t("emptyCart")}
          </h1>
          <p className="mt-2" style={{ color: config.mutedTextColor }}>
            {config.emptyDescription || t("emptyCartDescription")}
          </p>
          <div className="mt-6">
            <Button
              size="lg"
    ...
    >
      <div className={`${widthClass} mx-auto`}>
        <h1
          ...
          style={{ textAlign: config.alignment }}
        >
          {config.title || t("shoppingCart")}
        </h1>
        <div className="grid grid-cols-1 gap-8 lg:grid-cols-3">
          <div className="lg:col-span-2">
            <div
              className={`${radiusClass} divide-y border`}
          ...
          <div className="lg:col-span-1">
            <div
              className={`${radiusClass} sticky top-24 border p-6`}
              style={{ backgroundColor: config.cardBackgroundColor }}
            >
              <h2 className="text-lg font-medium">{tc("orderSummary")}</h2>
              <dl className="mt-6 space-y-4">
                <div className="flex justify-between">
                  <dt style={{ color: config.mutedTextColor }}>
                    {tc("subtotal")}
                  </dt>
                ...
                <div className="flex justify-between border-t pt-4">
                  <dt className="text-lg font-medium">{tc("total")}</dt>
                  <dd className="text-lg font-bold">{cart.display_total}</dd>
                </div>
                {cart.gift_card &&
                ...
                {cart.amount_due &&
                  ...
                  parseFloat(cart.amount_due) > 0 && (
                    <div className="flex justify-between border-t pt-4">
                      <dt className="text-lg font-medium">{t("amountDue")}</dt>
