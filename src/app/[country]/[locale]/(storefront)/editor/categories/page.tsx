...
function collectLeafCategories(
  ...
  parents: string[] = [],
): Array<CategoryNode & { labelPath: string }> {
...
export default async function CategoriesEditorPage({ params }: Props) {
  ...
  const categories = collectLeafCategories(
    (response.data ?? []) as CategoryNode[],
  );
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="mx-auto max-w-5xl px-6 py-10">
        ...
        <p className="mt-2 text-gray-600">
          Selecciona un tag para abrir directamente su editor visual.
        </p>
        <div className="mt-8 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
          {categories.map((category) => (
            <Link
              key={category.id}
              href={`${basePath}/editor/category/${category.permalink}`}
