...
const policies: Record<string, () => LocalLegalPolicy> = {
  ...
  "returns-policy": () => ({
    ...
    body_html: `
      ...
      <p>En determinados productos coleccionables —especialmente sobres, cajas, displays y productos TCG de contenido aleatorio— la condición de <strong>nuevo, íntegro y sellado</strong> constituye una parte esencial de su valor comercial. Abrir el precinto va más allá de lo que normalmente sería posible hacer al examinar ese producto en una tienda física y puede impedir que vuelva a venderse como el mismo producto nuevo y sellado.</p>
      <p>Cuando la apertura o manipulación produzca una pérdida objetiva de valor, el consumidor responderá de esa disminución en los términos legalmente previstos. <strong>Si el producto abierto ya no tiene un valor de reventa razonable como el bien originalmente contratado y la pérdida de valor es total y acreditable, la depreciación podrá alcanzar el 100 % del valor del artículo y, por tanto, el importe a reembolsar por ese artículo podrá ser de 0 €.</strong> No aplicaremos automáticamente ese resultado por el mero hecho de existir un precinto: valoraremos el tipo de producto, su estado y la pérdida de valor realmente producida.</p>
  ...
  "terms-of-service": () => ({
    ...
    body_html: `
      ...
      <p>En productos cuyo valor comercial depende de permanecer nuevos y sellados, abrir el precinto puede constituir una manipulación superior a la necesaria para examinarlos y causar una pérdida de valor muy elevada. Cuando la pérdida objetiva y acreditable sea total porque el artículo abierto ya no pueda revenderse razonablemente como el mismo producto contratado, <strong>la responsabilidad por disminución de valor podrá alcanzar el 100 % del precio de ese artículo</strong>. El cálculo se hará atendiendo al producto y estado concretos, no mediante una penalización fija.</p>
