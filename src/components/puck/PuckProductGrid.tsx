...
function matchesVariantIds(
  product: ProductWithRelations,
  ids: string[],
): boolean {
  if (ids.length === 0) return true;
  return (product.variants ?? []).some((variant) =>
    Boolean(variant.id && ids.includes(variant.id)),
  );
}
function matchesCategoryIds(
  ...
  return (product.categories ?? []).some(
    (category) =>
      ids.includes(category.id ?? "") || ids.includes(category.permalink ?? ""),
  );
}
function matchesVariantText(
  product: ProductWithRelations,
  query: string,
): boolean {
...
export function PuckProductGrid({
  ...
  const filteredProducts = (products as ProductWithRelations[]).filter(
    (product) => {
      if (
      ...
      if (productFilter === "available" && product.purchasable === false)
        return false;
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        {(title || subtitle) && (
          <div className="mx-auto mb-6 max-w-3xl text-center sm:mb-8">
            {title && (
              <h2
                className="text-2xl font-bold tracking-[-0.025em] sm:text-3xl md:text-[2rem]"
            ...
            {visibleProducts.map((product) => {
              ...
              return (
                <ProductCard
                  ...
                  url={productUrl}
                  badge={getMerchandisingBadge(product)}
