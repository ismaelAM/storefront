    ...
    <Sheet open={open} onOpenChange={handleOpenChange}>
      ...
      >
        <div className="relative h-5 w-5">
          <span
            className={`absolute left-0 right-0 top-[2px] h-0.5 rounded-full bg-current ${
              hasInteracted ? (open ? "animate-hamburger-top-open" : "animate-hamburger-top-close") : ""
            }`}
          />
          <span
            className={`absolute left-0 right-0 top-1/2 h-0.5 -translate-y-1/2 rounded-full bg-current ${
              hasInteracted ? (open ? "animate-hamburger-mid-open" : "animate-hamburger-mid-close") : ""
            }`}
            style={hasInteracted && !open ? { animationDelay: "0.2s", opacity: 0 } : undefined}
          />
          <span
            className={`absolute bottom-[2px] left-0 right-0 h-0.5 rounded-full bg-current ${
              hasInteracted ? (open ? "animate-hamburger-bottom-open" : "animate-hamburger-bottom-close") : ""
            }`}
          />
        </div>
      </Button>
      ...
      >
        ...
        <div className="hidden md:flex h-16 items-center justify-between border-b border-gray-200 px-4">
          {currentPanel.kind === "main" ? (
            ...
            <button type="button" onClick={popPanel} className="flex items-center gap-2 text-base font-semibold text-gray-700">
              <ArrowLeft className="h-5 w-5" />
              <span>{currentPanel.category.name}</span>
            </button>
          )}
          <Button variant="ghost" size="icon-sm" onClick={() => setOpen(false)} className="ml-auto cursor-pointer">
            <X className="size-4" />
          </Button>
        </div>
        ...
        <div className="relative flex-1 overflow-hidden">
          <div
            className={`absolute inset-0 flex flex-col bg-white transition-transform duration-300 ease-in-out ${
              animatedIndex === 0 && currentPanel.kind === "main" ? "translate-x-0" : "-translate-x-full"
            }`}
          >
            <nav className="flex flex-1 flex-col gap-1 overflow-y-auto px-4 pt-2">
              <Link href={basePath || "/"} onClick={() => setOpen(false)} className={linkClass}>
                {t("home")}
              </Link>
              ...
              <details className="group">
                ...
                <div className="ml-3 mt-1 flex flex-col gap-1 border-l border-gray-200 pl-2">
                  <Link href={`${basePath}/products`} onClick={() => setOpen(false)} className={linkClass}>
                    {t("allProducts")}
                  </Link>
              ...
              <Link href={`${basePath}/#contact`} onClick={() => setOpen(false)} className={linkClass}>
                {t("contact")}
              </Link>

              <div className="mt-2 border-t border-gray-200 pt-2">
                {wholesaleEnabled && (
                  <Link href={`${basePath}/wholesale`} onClick={() => setOpen(false)} className={`${linkClass} block`}>
                    {t("wholesale")}
                  </Link>
                )}
                <SheetClose asChild>
                  <Link href={`${basePath}/account`} className={`${linkClass} block`}>
                    {t("myAccount")}
                  </Link>
                </SheetClose>
              </div>
            ...
            <SheetFooter className="items-center gap-2 border-t border-gray-200 pt-4 lg:hidden">
          ...
          {panelStack.map((panel, index) => {
            ...
            const translateClass = !isAnimatedIn
              ? "translate-x-full"
            ...
            return (
              ...
              >
                <div className="border-b border-gray-200 px-4 py-2 md:hidden">
                  <button type="button" onClick={popPanel} className="flex items-center gap-2 py-2 text-base font-medium text-gray-700">
                    <ArrowLeft className="h-5 w-5" />
                ...
                <div className="border-t border-gray-200 px-4 py-3">
                  <Link
                    ...
                    onClick={() => handleOpenChange(false)}
                    className="block w-full py-2 text-center text-sm text-gray-500 transition-colors hover:text-gray-900"
                  >
