...
function getProduct(
  products: ReturnType<typeof usePuckProducts>["products"],
  position: number,
) {
    ...
    <section className={`w-full ${paddingClass}`} style={{ backgroundColor }}>
      <div className="container mx-auto grid items-center gap-6 px-4 sm:px-6 sm:gap-8 lg:grid-cols-2 lg:gap-10 lg:px-8">
        <div className={imageFirst ? "" : "lg:order-2"}>
          <div
            className={`relative w-full overflow-hidden bg-muted ring-1 ring-black/5 ${getPuckRadiusClass("large")} ${aspectClass}`}
          >
            ...
            {resolvedBadge && (
              <span
                className="absolute left-3 top-3 rounded-full px-3 py-1.5 text-xs font-semibold shadow-sm sm:left-5 sm:top-5"
                style={{
        ...
        <div
          className={`${alignmentClass} max-w-xl ${alignment === "center" ? "mx-auto" : ""}`}
        >
          <h2
            className="text-2xl font-bold leading-tight tracking-[-0.025em] sm:text-3xl md:text-4xl"
          ...
          <div
            className={`mt-5 flex flex-wrap items-baseline gap-x-3 gap-y-1 sm:mt-6 sm:gap-4 ${priceAlignmentClass}`}
          >
            <span
          ...
          {buttonText && (
            <Link
              href={productUrl}
              className="mt-6 inline-flex w-full items-center justify-center rounded-lg px-5 py-3 text-sm font-semibold shadow-sm transition-[opacity,transform] hover:-translate-y-0.5 hover:opacity-90 sm:mt-7 sm:w-auto sm:px-6"
              style={{ backgroundColor: buttonColor, color: buttonTextColor }}
            >
              {product.preorder ? "Reservar" : buttonText}
            </Link>
          )}
        </div>
      </div>
