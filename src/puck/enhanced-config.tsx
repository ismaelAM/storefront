  ...
  render: ({ field, value, onChange }: { field: { label?: string }; value?: string; onChange: (value: string) => void }) => (
    <FieldLabel label={field.label ?? label}>
      <div className="flex items-center gap-2">
        <input type="color" aria-label={field.label ?? label} value={typeof value === "string" && /^#[0-9a-fA-F]{6}$/.test(value) ? value : "#ffffff"} onChange={(event) => onChange(event.currentTarget.value)} className="h-9 w-12 cursor-pointer rounded border border-gray-300 bg-white p-1" />
...
export const enhancedConfig: Config = {
  ...baseConfig,
  categories: { ...(baseConfig.categories ?? {}), products: { title: "Productos", components: ["ProductGrid", "RealProductShowcase"] } },
  components: {
    ...baseComponents,
    ProductGrid: {
      label: "Productos",
      fields: {
        title: { type: "text", label: "Título" }, subtitle: { type: "textarea", label: "Subtítulo" },
        ...
        backgroundColor: colorField("Fondo"), cardBackgroundColor: colorField("Fondo de tarjeta"), titleColor: colorField("Color del título"), textColor: colorField("Color del texto"), priceColor: colorField("Color del precio"),
      },
    ...
    RealProductShowcase: {
      ...
      fields: {
        ...
        description: { type: "textarea", label: "Descripción" }, badge: { type: "text", label: "Etiqueta" }, buttonText: { type: "text", label: "Texto del botón" },
      ...
      defaultProps: { productPosition: 1, description: "", badge: "", buttonText: "Ver producto", backgroundColor: "#f3f4f6", titleColor: "#111827", textColor: "#4b5563", priceColor: "#111827", buttonColor: "#111827", buttonTextColor: "#ffffff", alignment: "left", imagePosition: "left", imageAspect: "square", padding: "large" } satisfies RealProductShowcaseProps,
